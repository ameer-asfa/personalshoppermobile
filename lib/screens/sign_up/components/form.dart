...
class _SignUpFormState extends State<SignUpForm> {
  ...
  Future<bool> _register(
      username, email, password, name, phone, address) async {
    var headers = {'Content-Type': 'application/json'};
    var request =
        http.Request('POST', Uri.parse('http://192.168.1.34:3000/sign_up/'));
  ...
  void _registerValidator(
      username, email, password, name, phone, address) async {
    var result =
        await _register(username, email, password, name, phone, address);
    if (result == true) {
